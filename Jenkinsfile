	...
	stage('Deploy') {
            steps {
             sshagent(['tomcat_deploy']) {
               sh '''scp -o StrictHostKeyChecking=no /var/lib/jenkins/workspace/Maven_depoly/target/maven-web-application.war root@54.178.14.36:/opt/apache-tomcat-8.5.84/webapps'''
            }
        }
	}
	}
